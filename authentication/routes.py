    ...
    if form.validate_on_submit():
        ...
        if user and bcrypt.check_password_hash(user.password, form.password.data):
            ...
            return (
                redirect(next_page)
                if next_page
                else redirect(url_for("arcticsun_index", device_type="arctic"))
            )
...
def send_reset_email(user):
    ...
    msg = Message(
        "Paswoord reset", sender=environ.get("MAIL_SENDER"), recipients=[user.email]
    )
...
def reset_request():
    ...
    if form.validate_on_submit():
        ...
        flash(
            "Er is een email verstuurd met instructies om uw paswoord te resetten.",
            "succes",
        )
        return redirect(url_for("login"))
